...
func (tidy *Tidy) AnchorAsName(val bool) (bool, error) {
	// C.TidyAnchorAsName = 87 it doesn't exists in tidyp
	return tidy.optSetBool(87, cBool(val))
}
...
func (tidy *Tidy) SortAttributes(val int) (bool, error) {
	switch val {
	case None, Alpha:
		// C.TidySortAttributes = 84 it doesn't exists in tidyp
		return tidy.optSetInt(84, (C.ulong)(val))
	}
